...
class FileOperator:
    ...
    def __init__(
        ...
        *,
        ...
        """Create a FileOperator instance for managing file operations.  To be used with the `with`
        statement. Operations are scheduled first, the performed with the `commit` method, or
        optionally automatically when the with block is exited.

        The context manager is not reentrant.

        :param parent: A HANDLE to the window that should own any dialog boxes that are displayed.
            Also supports wx.Window instances.
        :param flags: The FileOperationFlags to use for the operations.
        :param commit_on_exit: If True (default: False), `commit` will be performed automatically
    ...
    def move_file(
        ...
        """Schedule to move a file from `source` to `destination`.  Optionally rename the file in
        the process. NOTE: Windows performs a move to a different logical drive as a copy and
        delete.
    ...
    def move_files(self, sources: Iterable[StrPath], destination: StrPath) -> None:
        """Schedule to move the files `sources` to `destination`. NOTE: Windows performs a move to a
        different logical drive as a copy and delete.
    ...
    def rename_file(
        ...
        """Schedule to rename `source` to `new_name`.  If `allow_move` is True, then when the
        destination is in a different directory, this will perform a move instead.

        :param source: The file to be renamed.
        :param new_name: The new name of the file.
        :param allow_move: If True, then converts a rename into a move when new_name is in a
            different directory than the source.
        """
        # TODO: implement allow_move
        self.ifo.RenameItem(parse_filename(source), new_name)

    @convert_exceptions
    def rename_files(self, sources: Iterable[StrPath], new_name: StrPath) -> None:
        """Schedule to rename the files `sources` to `new_name`.  All files will be renamed to the
        new name.
    ...
    def copy_file(
        ...
        """Schedule to copy a file from `source` to `destination`. Optionally rename the file in the
        process.
    ...
    @convert_exceptions
    def commit(self):
        """Perform all schedulted file operations."""
